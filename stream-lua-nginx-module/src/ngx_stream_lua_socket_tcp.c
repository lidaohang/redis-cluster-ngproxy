...
static int ngx_stream_lua_socket_tcp_receive_retval_handler(
    ngx_stream_session_t *s, ngx_stream_lua_socket_tcp_upstream_t *u,
    lua_State *L);
static ngx_int_t ngx_stream_lua_socket_read_redis(void *data, ssize_t bytes);
...
ngx_stream_lua_socket_tcp_receive(lua_State *L)
{
    ...
    if (n > 1) {
        ...
        case LUA_TSTRING:
            ...
            switch (pat.data[1]) {
            case 'r':
                u->input_filter = ngx_stream_lua_socket_read_redis;
                break;
            case 'l':
...
static ngx_int_t
redis_parse_line(char *rdata, ssize_t rdata_len)
{
    const char                      *p = rdata;
    const char                      *last;
    int                             seen_cr = 0;
...
redis_proto_decode(char *rdata, ssize_t len)
{
    switch (*rdata)
    {
        ...
        case '+' :
        case ':' :
            return redis_parse_line(rdata, len);
        case '$' :
...
//add redis proto
static ngx_int_t
ngx_stream_lua_socket_read_redis(void *data, ssize_t bytes)
{
    ngx_stream_lua_socket_tcp_upstream_t      *u = data;
    ...
    rc = redis_proto_decode((char*)b->pos, bytes);
    if ( rc == NGX_OK ) {
        u->buf_in->buf->last += bytes;
        b->pos += bytes;
