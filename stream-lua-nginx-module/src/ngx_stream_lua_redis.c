...
#include "ngx_stream_lua_redis.h"
#include "ngx_stream_lua_redis_parser.h"


static int ngx_stream_lua_ngx_redis_slot(lua_State *L);
static int ngx_stream_lua_ngx_redis_decode(lua_State *L);

static void *redis_null = NULL;

void
ngx_stream_lua_inject_redis_api(lua_State *L)
{
    lua_pushcfunction(L, ngx_stream_lua_ngx_redis_slot);
    lua_setfield(L, -2, "redis_slot");

    lua_pushcfunction(L, ngx_stream_lua_ngx_redis_decode);
    lua_setfield(L, -2, "redis_decode");
...
static int
ngx_stream_lua_ngx_redis_slot(lua_State *L)
{
    size_t                  len;
    u_char                  *p;
    ...
    lua_pushnumber(L, (lua_Number) key_hash_slot((char*)p, len));
    return 1;
}


static int
ngx_stream_lua_ngx_redis_decode(lua_State *L)
{
    char                      *p;
    size_t                     len = 0;
    int                        n;
    ...
    n = lua_gettop(L);
    if (n != 1) {
        return luaL_error(L, "expecting one arguments but seen %d", n);
    }

    p = (char *) luaL_checklstring(L, 1, &len);

    return ngx_stream_lua_redis_parse(L, &p, len);
}
